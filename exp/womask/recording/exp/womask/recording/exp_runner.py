...
if __name__ == '__main__':
    ...
    parser.add_argument('--conf', type=str, default='config.conf')
    parser.add_argument('--mode', type=str, default='caus')
    ...
    if args.mode == 'train':#训练
        runner.train()
    elif args.mode == 'validate_mesh':#验证
        runner.validate_mesh(world_space=True, resolution=512, threshold=args.mcube_threshold)
    elif args.mode.startswith('interpolate'):  # Interpolate views given two image indices(插值视图)
        _, img_idx_0, img_idx_1 = args.mode.split('_')
        img_idx_0 = int(img_idx_0)
        img_idx_1 = int(img_idx_1)
        runner.interpolate_view(img_idx_0, img_idx_1)
    elif args.mode == 'caus':#渲染
